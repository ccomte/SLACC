...
int xik[MAX_N][2 * MAX_K];
/* Number of jobs of each type and of each size at each server:
 * xik[i][2*k] = number of type-k jobs with mean size 1 / kappa[2*k] at server i
 * xik[i][2*k+1] = number of type-k jobs with mean size 1 / kappa[2*k+1] at server i
 */

double theta;         /* total departure rate */
double thetai[MAX_N]; /* unnormalized departure rate from each server */
double thetaik[MAX_N][2 * MAX_K];
/* unnormalized departure rate of the jobs of each type and size from each server */
...
double psii[MAX_N];   /* empirical proportion of time each server is idle */
...
static inline void update_departure (int i) {
  ...
  /* update the departure rate from server i */
  thetai[i] = 0.;
  if (xi[i] > 0) {
    for (k = 0 ; k < K ; ++k) {
...
void read_inputs (int argc, char **argv) {
  ...
  /* read arguments */
  opterr = 0;
  while ( (c = getopt(argc, argv, "f:k:n:l:c:r:s:m:d:R:w:t:h")) != -1 ) {
    switch(c) {
      case 'f':
        /* output file name */
      ...
      case 'h':
        ...
        printf("\nOptions:\n"
            ...
            "-r: per-type traffic intensities and per-server capacities\n"
...
void print_inputs () {
  ...
  printf("\n--- DYNAMIC - BIPARTITE - SIMULATION - HYPEREXPONENTIAL ---\n\n");
